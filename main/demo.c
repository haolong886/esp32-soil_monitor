...
static subscription_t subscription_list[] = {
    {TOPIC_CMND_PLANT_MOISTURE, 0, false, handle_cmnd_plant_moisture},
...
static volatile plant_state_t s_plant_state = {
    .power = PLANT_MOISTURE_OFF,
    .on_time = 500,
    .off_time = 500
};
...
void store_plant_state_to_nvs(nvs_handle_t handler, plant_state_t *plant_state)
{
    set_value_to_nvs(handler, "power", &plant_state->power, NVS_TYPE_U8);
    esp_err_t err = nvs_commit(handler);
    ESP_LOGI(TAG, "committing updates in NVS ... %s", (err != ESP_OK) ? "Failed!" : "Done!");
}
...
void init_plant_state(void)
{
    nvs_handle_t handler;

    esp_err_t err = nvs_open(PLANT_STORAGE_NAMESPACE, NVS_READWRITE, &handler);
    if(err != ESP_OK) {
        ESP_LOGW(TAG, "error (%s) opening NVS handle!", esp_err_to_name(err));
    }
    else {
        get_value_from_nvs(handler, "power", (void *)&s_plant_state.power, NVS_TYPE_U8);
    }
}
...
void sync_plant_state(esp_mqtt_client_handle_t client, plant_state_t *st)
{
    sprintf(s_out_msg_plant, "%hu", st->moisture);
    if(esp_mqtt_client_enqueue(client, TOPIC_STAT_PLANT_MOISTURE, s_out_msg_plant, 0, QOS_1, RETAIN, true) >= 0) {
        ESP_LOGI(TAG, "publist to %s: %s", TOPIC_STAT_PLANT_MOISTURE, s_out_msg_plant);
    }
}

void plant_task(esp_mqtt_client_handle_t client)
{

    plant_state_t plant_state;
    uint8_t power_state_changing_flag = 0;
    ...
    while(1) {
        if(xQueueReceive(s_plant_state_queue, &plant_state, 0) == pdTRUE) {
            ESP_LOGI(TAG, "plant task begin to handle msg");
            if (s_plant_state.power != plant_state.power) {
                s_plant_state.power = plant_state.power;
                power_state_changing_flag = 1;
                ESP_LOGI(TAG, "moisture sensor power state is changing to %d..., done.", s_plant_state.power);
                // TODO: save s_plant_state to NVC
            }
        }
        if (s_plant_state.power == PLANT_MOISTURE_ON)
        {
            int moisture = soil_moisture_sensor_read();    
            ESP_LOGI(TAG, "moisture voltage = %d", moisture);
            // 只有当湿度变化超过 10%, 或者收到 Power on 消息，才更新湿度值
            if (abs(moisture - s_plant_state.moisture)/ (float)s_plant_state.moisture >= 0.1
                || power_state_changing_flag) {
                s_plant_state.moisture = moisture;
                sync_plant_state(client, &s_plant_state);
                store_plant_state_to_nvs(handler, &s_plant_state);

                power_state_changing_flag = 0;
            }
        }
        vTaskDelay(PLANT_TASK_INTERVAL / portTICK_PERIOD_MS);
    }
}
...
esp_err_t send_plant_state_to_queue(plant_state_t *plant_state)
{
    if(xQueueSend(s_plant_state_queue, plant_state, 0) != pdTRUE) {
        ESP_LOGE(TAG, "failed to send to plant state queue");
        return ESP_FAIL;
    }
    return ESP_OK;
}
...
static esp_err_t handle_cmnd_plant_moisture(char *data, int data_len)
{
    plant_state_t plant_state = s_plant_state;
    if(!strncmp(data, PLANT_MOISTURE_ON_STR, data_len)) {
        plant_state.power = PLANT_MOISTURE_ON;
    }
    else if(!strncmp(data, PLANT_MOISTURE_OFF_STR, data_len)) {
        plant_state.power = PLANT_MOISTURE_OFF;
    }
    else {
    ...
    return send_plant_state_to_queue(&plant_state);
}
...
void app_main(void)
{
    ...
    s_plant_state_queue = xQueueCreate(10, sizeof(plant_state_t));
    if (!s_plant_state_queue) {
        ESP_LOGE(TAG, "create plant state queue failed");
        abort();
    }
    ...
    xTaskCreatePinnedToCore((TaskFunction_t)plant_task, "plant_task", 4096, client, CONFIG_PLANT_TASK_PRIORITY, NULL, 0);
